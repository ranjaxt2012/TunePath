...
const TABS = [
  ...
  { name: 'create', label: '', icon: 'add', iconActive: 'add', isCenter: true },
    ...
    <View style={[styles.tabBar, { backgroundColor: theme.background, borderTopColor: theme.border, paddingBottom: Math.max(insets.bottom, 8) }]}>
      {TABS.map((tab, index) => {
        ...
        if ('isCenter' in tab && tab.isCenter) {
          return (
            <TouchableOpacity key={tab.name} style={[styles.centerBtn, { backgroundColor: theme.primary, shadowColor: theme.primary }]} onPress={() => navigation.navigate(tab.name)} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  ...
  centerBtn: { width: 52, height: 52, borderRadius: 16, alignItems: 'center', justifyContent: 'center', marginTop: -20, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 8, elevation: 8 },
});
